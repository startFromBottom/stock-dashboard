...
import { STAPLES_LAYERS, STAPLES_FLAG_BY_NAME } from '@/data/staplesCompanies';
...
function StaplesChainDiagram({ activeId, onSelect }) {
  return (
    <div style={{ width: '100%', maxWidth: SVG_W }}>
      <svg
        viewBox={`0 0 ${SVG_W} ${TOTAL_H}`}
        xmlns="http://www.w3.org/2000/svg"
        style={{ display: 'block', width: '100%', borderRadius: 12, background: '#0a0e0a' }}
      >
        <defs>
          <pattern id="stgrid" width="20" height="20" patternUnits="userSpaceOnUse">
            <path d="M 20 0 L 0 0 0 20" fill="none" stroke="#1a1f1a" strokeWidth="0.4" />
          </pattern>
          <filter id="stglow" x="-30%" y="-30%" width="160%" height="160%">
            <feGaussianBlur stdDeviation="3.5" result="blur" />
            <feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge>
          </filter>
          <marker id="starrow" markerWidth="7" markerHeight="7" refX="4" refY="3.5" orient="auto">
            <path d="M0,0 L7,3.5 L0,7 z" fill="#1a2f22" />
          </marker>
          <marker id="starrow-a" markerWidth="7" markerHeight="7" refX="4" refY="3.5" orient="auto">
            <path d="M0,0 L7,3.5 L0,7 z" fill="#34d399" />
          </marker>
        </defs>

        <rect width={SVG_W} height={TOTAL_H} fill="url(#stgrid)" />
            ...
            <g key={layer.id} onClick={() => onSelect(layer.id)} style={{ cursor: 'pointer' }}>
              {li > 0 && (
                <line
                  ...
                  x2={centerX} y2={y}
                  stroke={isActive || activeId === CHAIN_LAYERS[li - 1].id ? '#34d399' : '#1a2f22'}
                  strokeWidth={isActive ? 2 : 1.4}
                  strokeDasharray={isActive ? 'none' : '4 3'}
                  markerEnd={isActive ? 'url(#starrow-a)' : 'url(#starrow)'}
...
/* ═══════════════════════════════════════════════════════
   사이드 패널
═══════════════════════════════════════════════════════ */
function StaplesSidePanel({ activeId, onSelect }) {
  return (
...
function StaplesCompanyPanel({ layer }) {
...
/* ═══════════════════════════════════════════════════════
   메인 Staples Dashboard
═══════════════════════════════════════════════════════ */
export default function StaplesDashboard() {
  const [activeId, setActiveId] = useState(null);
  const toggle = id => setActiveId(prev => prev === id ? null : id);

  const activeLayer = STAPLES_LAYERS.find(l => l.id === activeId);
    ...
    <div>
      <div style={{ marginBottom: 14 }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.7 }}>
          <span style={{ color: '#fbbf24' }}>농업/원료</span>
          &ensp;→&ensp;
          <span style={{ color: '#38bdf8' }}>식품/음료</span>
          &ensp;→&ensp;
          <span style={{ color: '#f87171' }}>담배/주류</span>
          &ensp;→&ensp;
          <span style={{ color: '#34d399' }}>생필품/유통</span>
        </p>
      </div>

      <div className="illust-wrap" style={{ alignItems: 'flex-start' }}>
        <div className="illust-svg" style={{ flex: '1 1 auto' }}>
          <StaplesChainDiagram activeId={activeId} onSelect={toggle} />
        </div>
        <StaplesSidePanel activeId={activeId} onSelect={toggle} />
      </div>
      ...
      {activeLayer && (
        <div style={{ marginTop: 24 }}>
          <StaplesCompanyPanel layer={activeLayer} />
        </div>
      )}
    </div>
  );
