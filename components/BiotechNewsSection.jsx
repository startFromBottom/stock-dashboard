...
import {
  BIOTECH_NEWS_ITEMS,
  BIOTECH_NEWS_TYPE_LABEL,
  BIOTECH_CATEGORY_LABEL,
  countBiotechByCat,
} from '@/data/biotech-news';

const ALL_CATS = Object.keys(BIOTECH_CATEGORY_LABEL);

const GROUPS = [
  { id: 'all',       label: '전체',                cats: ALL_CATS },
  { id: 'upstream',  label: '🤖 업스트림',          cats: ['ai_dd', 'genomics_seq'] },
  { id: 'modality',  label: '💊 치료 모달리티',     cats: ['gene_cell', 'antibody'] },
  { id: 'services',  label: '🏥 서비스 인프라',     cats: ['cro_service', 'cdmo_mfg'] },
  { id: 'pharma',    label: '💰 빅파마·진단',       cats: ['pharma_major', 'dx_device'] },
];

export default function BiotechNewsSection() {
  ...
  const catCount = useMemo(() => countBiotechByCat(BIOTECH_NEWS_ITEMS), []);
  ...
  const groupCount = useMemo(() => {
    const result = { all: BIOTECH_NEWS_ITEMS.length };
    for (const g of GROUPS) {
      if (g.id !== 'all') {
        result[g.id] = BIOTECH_NEWS_ITEMS.filter(n => g.cats.includes(n.category)).length;
      }
    }
    return result;
  }, []);
  ...
  const filtered = useMemo(() => {
    const result = BIOTECH_NEWS_ITEMS.filter(n => {
    ...
    <div>
      ...
      <div className="news-live-bar">
        <span className="news-total-count">
          총 {BIOTECH_NEWS_ITEMS.length}개 뉴스·리포트
          &ensp;·&ensp;
          리포트 {BIOTECH_NEWS_ITEMS.filter(n => n.type === 'report').length}개
          &ensp;·&ensp;
          뉴스 {BIOTECH_NEWS_ITEMS.filter(n => n.type === 'news').length}개
        </span>
      </div>
      ...
      <div className="news-comp-filters">
        ...
        >
          전체 ({groupCount[activeGroup] ?? BIOTECH_NEWS_ITEMS.length})
        </button>
        {catsWithNews.map(catId => {
          const info = BIOTECH_CATEGORY_LABEL[catId];
          if (!info) return null;
          return (
            <button
              key={catId}
      ...
      <div className="news-type-bar">
        <div className="news-type-filters">
          {['all', 'news', 'report'].map(t => (
            ...
            >
              {t === 'all' ? '전체' : BIOTECH_NEWS_TYPE_LABEL[t]?.label}
            </button>
          ))}
        </div>
        <button
      ...
      {filtered.length === 0 ? (
        ...
        <div className="news-list">
          {filtered.map(item => {
            const catInfo  = BIOTECH_CATEGORY_LABEL[item.category];
            const typeInfo = BIOTECH_NEWS_TYPE_LABEL[item.type];
            return (
      ...
      <p className="hint-text" style={{ marginTop: 24 }}>
        바이오테크·AI 신약개발 섹터 뉴스 {BIOTECH_NEWS_ITEMS.length}개 · 2025~2026년 기준
      </p>
    </div>
  );
}
