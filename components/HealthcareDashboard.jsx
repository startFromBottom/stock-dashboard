...
const CHAIN_LAYERS = [
  {
    ...
    nodes: ['수술 로봇', 'AI 영상진단', '의료 센서'],
  },
  {
    id: 'medical_it',
    label: '의료IT & 디지털헬스',
    icon: '💻',
    color: { bg: '#0a1820', bd: '#8b5cf6', bgA: '#2d1f4f', bdA: '#c084fc', tx: '#c084fc', txA: '#e9d5ff' },
    nodes: ['EHR', '웨어러블', '원격진료'],
  },
  {
...
function HealthcareChainDiagram({ activeId, onSelect }) {
  return (
    <div style={{ width: '100%', maxWidth: SVG_W }}>
      ...
      >
        <defs>
          <pattern id="hcgrid" width="20" height="20" patternUnits="userSpaceOnUse">
            <path d="M 20 0 L 0 0 0 20" fill="none" stroke="#0a1f18" strokeWidth="0.4" />
          </pattern>
          <filter id="hcglow" x="-30%" y="-30%" width="160%" height="160%">
          ...
          return (
            <g key={layer.id} onClick={() => onSelect(layer.id)} style={{ cursor: 'pointer' }}>
              ...
              {layer.nodes.map((node, ni) => {
                const nodeX = PAD_X + 24 + (ni % 3) * 140;
                const nodeY = y + (Math.floor(ni / 3) === 0 ? 38 : 55);
...
/* ═══════════════════════════════════════════════════════
   사이드 패널
═══════════════════════════════════════════════════════ */
function HealthcareSidePanel({ activeId, onSelect }) {
  return (
...
function HealthcareCompanyPanel({ layer }) {
...
export default function HealthcareDashboard() {
  ...
  const activeLayer = HEALTHCARE_LAYERS.find(l => l.id === activeId);
    ...
    <div>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.7 }}>
          <span style={{ color: '#38bdf8' }}>의료기기</span>
          &ensp;→&ensp;
          <span style={{ color: '#c084fc' }}>의료IT & 디지털헬스</span>
          &ensp;→&ensp;
          <span style={{ color: '#f87171' }}>의료 서비스 & 인프라</span>
          &ensp;→&ensp;
          <span style={{ color: '#34d399' }}>헬스케어 SaaS & 결제</span>
        </p>
      </div>
      ...
      <div className="illust-wrap" style={{ alignItems: 'flex-start' }}>
        <div className="illust-svg" style={{ flex: '1 1 auto' }}>
          <HealthcareChainDiagram activeId={activeId} onSelect={toggle} />
        </div>
        <HealthcareSidePanel activeId={activeId} onSelect={toggle} />
      </div>
      ...
      {activeLayer && (
        <div style={{ marginTop: 24 }}>
          <HealthcareCompanyPanel layer={activeLayer} />
        </div>
      )}
    </div>
  );
